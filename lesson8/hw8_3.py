...
def fill_table_teacher():
    result = jinja2.Template("""INSERT INTO teacher(id_teacher, fist_name,second_name,last_name,phone,email,address,grade) VALUES
            (1, 'Ivan', 'Ivanovich', 'Ivanenko', '0962179990', 'Ivanenko@yahoo.com', 'Kijow, ul. Chreszczatik, 7', 'Assistent'),
            (2, 'Petro', 'Petrovich', 'Petrenko', '0735160913', 'Petrenko@yahoo.com', 'Kijow, ul. Wolodymyrska, 13', 'Doctor of philosophy'),
            (3, 'Sydor', 'Sydorovich', 'Sydorenko', '0502203484', 'Sydorenko@yahoo.com', 'Kijow, ul.Industrialna, 23', 'Doctor of sciences'); """)
...
def fill_table_groups():
    temp_name = ""
    result = "INSERT INTO groups(name_group,id_teacher,id_subject,id_student,punkts_by_subject,timepunkts_by_subject) VALUES"
    for i in range(1, 31):  
        for j in range(1, 6):
            for k in range(1, 4):
                if i <= 10:
                    ...
                    temp_name = 'Tech103'
                temp_punkt = random.randint(0, 100)
                temp_data_day = random.randint(1,30)
                temp_data_month = random.randint(1,12)
                temp_data_year = random.randint(2000, 2021)
